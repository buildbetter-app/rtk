// Modified by BuildBetter: moved the shared CLI dispatch into the library target.
...
fn run_fallback(parse_error: clap::Error, args: &[OsString]) -> Result<i32> {
    ...
    if core::constants::RTK_META_COMMANDS.contains(&display_args[0].as_str()) {
        parse_error.print()?;
        return Ok(parse_error.exit_code());
    }

    let raw_command = display_args.join(" ");
    ...
    let lookup_cmd = {
        let base = std::path::Path::new(&display_args[0])
            .file_name()
            .map(|n| n.to_string_lossy().into_owned())
            .unwrap_or_else(|| display_args[0].clone());
        std::iter::once(base.as_str())
            .chain(display_args[1..].iter().map(|s| s.as_str()))
            .collect::<Vec<_>>()
            .join(" ")
    };
    ...
    if let Some(filter) = toml_match {
        ...
        let result = if filter.filter_stderr {
            ...
            core::utils::resolved_command(&display_args[0])
                .args(&args[1..])
        ...
        let status = core::utils::resolved_command(&display_args[0])
            .args(&args[1..])
            .stdin(std::process::Stdio::inherit())
...
/// Run the process-level `rtk` binary entry point.
pub fn main_entry() {
...
/// Parse the process arguments and run the `rtk` CLI.
pub fn run_cli() -> Result<i32> {
    // Fire-and-forget telemetry ping (1/day, non-blocking)
    core::telemetry::maybe_ping();

    let process_args: Vec<OsString> = std::env::args_os().collect();
    let original_argv = &process_args[1..];
    let cli = match Cli::try_parse_from(&process_args) {
        Ok(cli) => cli,
        Err(e) => {
            ...
            return run_fallback(e, original_argv);
        }
    };
...
///
/// The slice must not include an `rtk` executable name. For example, pass
/// `["git", "status"]`, not `["rtk", "git", "status"]`.
pub fn run_filtered(argv: &[OsString], verbose: u8) -> Result<i32> {
    run_filtered_with_options(
        argv,
        RunOptions {
            verbose,
            tracking: true,
...
/// Options for an embedded RTK command execution.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub struct RunOptions {
    /// Verbosity passed to the same command handlers used by the binary.
    pub verbose: u8,
...
pub fn run_filtered_with_options(argv: &[OsString], options: RunOptions) -> Result<i32> {
    ...
    let run = || {
        let mut cli = match Cli::try_parse_from(&parse_args) {
            Ok(cli) => cli,
            Err(e) => {
                if matches!(e.kind(), ErrorKind::DisplayHelp | ErrorKind::DisplayVersion) {
                    e.print()?;
    ...
    if options.tracking {
        ...
        core::tracking::without_tracking(run_with_argv)
    }
}
...
#[cfg(unix)]
#[test]
fn embedded_unknown_command_uses_supplied_argv_without_tracking() {
    let code = run_filtered_with_options(
        &[OsString::from("true")],
        RunOptions {
            verbose: 0,
            tracking: false,
